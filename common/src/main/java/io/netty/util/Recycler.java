...
public abstract class Recycler<T> {
    ...
    private final FastThreadLocal<Stack<T>> threadLocal = new FastThreadLocal<Stack<T>>() {
        ...
        protected Stack<T> initialValue() {
            return new Stack<T>(Recycler.this, Thread.currentThread(),
                    maxCapacity, maxSharedCapacityFactor, maxDelayedQueues);
        }
    };
    ...
    protected Recycler(int maxCapacity, int maxSharedCapacityFactor) {
        this(maxCapacity, maxSharedCapacityFactor, MAX_DELAYED_QUEUES);
    }

    protected Recycler(int maxCapacity, int maxSharedCapacityFactor, int maxDelayedQueues) {
        if (maxCapacity <= 0) {
            this.maxCapacity = 0;
    ...
    static final class Stack<T> {
        ...
        Stack(Recycler<T> parent, Thread thread, int maxCapacity, int maxSharedCapacityFactor, int maxDelayedQueues) {
            this.parent = parent;
            this.thread = thread;
            this.maxCapacity = maxCapacity;
